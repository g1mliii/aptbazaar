-- Phase 4 forward migration for databases already migrated through 0021.
--
-- Replaces place_order with the optimized hot-path implementation: no per-call
-- temp table, no full PII row projection, duplicate product aggregation remains,
-- and malformed carts are bounded before price recomputation.
--
