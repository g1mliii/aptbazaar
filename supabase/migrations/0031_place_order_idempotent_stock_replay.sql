-- Phase 6 forward migration for environments that already applied 0030 before the same-key
-- last-unit replay fix. Replaces place_order only; function signature and generated TypeScript
-- types are unchanged.
--
-- Fix: a duplicate submit with the same (store_id, idempotency_key) can wait behind the first
-- transaction's product row locks. Once the first order commits and consumes the last unit, the
-- duplicate must replay the existing order instead of raising STP05 from the now-depleted stock
-- check. This version re-checks idempotency after product locks are acquired and before
-- stock-derived rejects.
