-- Scale follow-up: fold the founder dashboard's aggregation into Postgres.
--
-- loadAdminMetrics() used to pull *every* paid order row (store_id, total_cents) into the Worker and
-- sum/group them in JS (lib/admin/metrics.ts computeAdminMetrics). Correct at MVP scale but the fetch
-- — and the Worker memory it needs — grows linearly with lifetime order volume. This RPC does the
-- whole rollup in SQL and returns a single bounded JSON document instead.
--
-- The platform-fee RATE stays single-sourced in TypeScript (lib/pricing/fee.ts PLATFORM_FEE_BPS): the
-- caller passes it in as p_fee_bps, so this function never hard-codes "3%". Only the per-order rounding
-- shape is reproduced here, and `round(numeric)` rounds half away from zero — identical to JS
-- Math.round for the non-negative totals paid orders always carry.
--
-- Cross-tenant by design (the founder needs a platform-wide view RLS would otherwise scope to one
-- seller), so it is SECURITY DEFINER and granted to service_role only — anon/authenticated can never
-- call it. It reads only store_id + total_cents off orders (no PII; no select *).
...
as $$
  with paid as (
    select
      o.store_id,
      o.total_cents,
      -- Per-order fee, rounded then summed — matches what Stripe took on each charge. The rate comes
      -- from the caller (PLATFORM_FEE_BPS); never re-derive the % here.
